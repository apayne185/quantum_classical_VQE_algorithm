...
def serial_vqe(mol_name, coords, fci_ref, reps=1, max_iterations=500, seed=42):
    ...
    # Match stack: adaptive reps for larger molecules
    adaptive_reps = min(reps + 1, 3) if n_qubits > 4 else reps
    ansatz = EfficientSU2(n_qubits, reps=adaptive_reps, entanglement="full").decompose()
    n_params = ansatz.num_parameters
    # Scale iterations by parameter count — same as stack
    ...
    for k in range(1, max_iterations + 1):
        ...
        # Track best physical energy (above FCI)
        if current >= fci_ref:
    ...
    # Report best physical energy if final crossed below FCI
